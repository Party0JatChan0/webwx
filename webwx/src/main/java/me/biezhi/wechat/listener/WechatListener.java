...
public class WechatListener {
	...
	public void start(final WechatService wechatService, final WechatMeta wechatMeta, final int automsgflag, final String automsg){
				...
				while(true){
					...
					if(arr[0] == 0){
						if(arr[1] == 2){
							...
							wechatService.handleMsg(wechatMeta, data, automsgflag, automsg);
						} else if(arr[1] == 6){
							JSONObject data = wechatService.webwxsync(wechatMeta);
							wechatService.handleMsg(wechatMeta, data, automsgflag, automsg);
						} else if(arr[1] == 7){
